...
**日期**: 2026-04-30
**作者**: 用户 + AI
**状态**: 已交付 · **已归档**（见同目录上一级 [`README.md`](../README.md)「执行计划归档」条目；OpenAPI 日常维护：`server/src/docs/openapi.ts`）
**关联任务列表**: [`tasks.md`](./tasks.md)

## 1. 背景

本需求已归档；日常以根目录 README / `docs/README.md` 中的 API 文档地址与 `server/src/docs/openapi.ts` 为准。
...
- [x] 包含 `openapi`, `info`, `servers`, `tags`, `components`, `paths`。
- [x] 每个 REST 路径至少包含 summary、description、参数/requestBody、成功响应、错误响应。
- [x] 统一错误响应 schema 为 `{ error: { code, message, details } }`。
...
- [x] 页面标题明确为 Edge Muse Platform API Reference。
- [x] 页面从 `/api/openapi.json` 加载 spec。
- [x] 文档页 CSP 允许 Scalar 所需 CDN 与 inline 初始化脚本，不影响主应用 CSP。
...
- [x] 标注公开接口、登录态接口、admin 接口、sysadmin 接口。
- [x] 标注非安全方法需要 `X-CSRF-Token`。
- [x] 生产环境文档访问策略有明确实现或说明。
...
| 2026-05-06 | —    | 需求与 tasks 迁至 `docs/archive/exec-plans-api-docs-support/`；F1–F3 验收勾选与正文对齐（已交付）。 |
