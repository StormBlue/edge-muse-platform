...
截至 2026-04-28，Phase 1-4 已完成：`/ai-image` 独立页面、案例库 D1 管理、AI Prompt 助手、生成入口 A/B 测试管理均已落地。Phase 5 已完成本地验证与专项测试补强，实际灰度发布仍需在生产环境通过 sysadmin 实验页面执行。
...
- 提供灰度预设按钮：回滚旧版、并列展示、当前范围强制新版、25% B 变体；当前范围强制新版必须先配置 `userIds` 或 `adminIds` 等目标范围。
- 保存前显示风险摘要：无效 JSON 阻止误保存，全量强制新版标为高风险，高流量 B 变体给出警示，且仅配置 `excludeUserIds` 仍按全量范围提示。
...
{
  "userIds": ["usr_xxx"],
  "adminIds": ["usr_admin_xxx"],
  "excludeUserIds": ["usr_skip_xxx"]
}
```

空对象表示全部非 sysadmin 用户；`userIds` 是指定用户白名单；`adminIds` 表示指定 admin 创建的用户；`excludeUserIds` 用于从命中范围中排除用户。
