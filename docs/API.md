...
| Experiments  | `POST /experiments/events` (authenticated client experiment events; rate limited)           |
| Images       | `GET /i/:imageId`, `POST /uploads`                                                          |
| Admin        | `/admin/users`, quota, usage, and manual user password reset endpoints                      |
| Sysadmin     | `/sysadmin/keys`, `/sysadmin/experiments/generation`, admins, dashboard, session inspection |
