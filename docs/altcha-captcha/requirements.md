...
本期要引入一个仍在维护、业界可接受、可自托管的开源验证码 provider。运行环境是 Cloudflare Workers，CPU 预算有限，因此方案需要避免服务端生成图片、复杂 ML/行为分析或高成本服务端求解。调研后选用 ALTCHA Widget v3 + `SHA-256` PoW challenge：浏览器端做 proof-of-work，Worker 只负责签发 challenge、校验 HMAC/solution 和 replay 消费。

### 1.1 开源项目调研

| 项目                                                                                            | 开源/维护                                         | 特点                                                                                        | Cloudflare Worker 适配判断                                                                                |
| ----------------------------------------------------------------------------------------------- | ------------------------------------------------- | ------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| [ALTCHA](https://github.com/altcha-org/altcha) / [Docs](https://altcha.org/docs/v2/)            | MIT；Widget v3 已发布，官方文档近期更新           | 自托管、隐私优先、PoW、无第三方请求；v3 widget 支持 `SHA-256`、PBKDF2、Argon2/Scrypt 等形态 | 最适合。使用 `SHA-256` challenge 时 Worker 只做常数次 HMAC/SHA-256 和 replay 消费，PoW 循环在浏览器       |
...
- ✅ Worker 端生成、签名并校验 ALTCHA challenge，使用 Durable Object 做生产防重放，KV 作为本地/测试 fallback。
...
- Worker 端 challenge 签发与校验不依赖 D1；只使用环境密钥、Web Crypto 和 replay 存储。
...
**行为**: Worker 生成随机 nonce/salt、expiresAt、目标 keyPrefix 和 HMAC signature；难度来自配置；接口仅在当前地区 provider 为 `altcha` 时签发，并带匿名限流。

**输出**: ALTCHA Widget v3 可消费的 challenge JSON。

**验收标准**:

- [ ] challenge 使用 Widget v3 原生 `{ parameters, signature }` 结构，包含 `algorithm`、`cost`、`data.difficulty`、`expiresAt`、`keyLength`、`keyPrefix`、`nonce`、`salt`。
...
**行为**: 解析 Widget v3 返回的 payload，校验签名、过期时间、solution 与 replay 消费。
...
- [ ] `docs/SECURITY.md` 说明 ALTCHA PoW 与 replay 消费。
...
- Worker 校验单次只做常数次 SHA-256/HMAC 和一次 replay 消费，避免服务端 PoW。
...
- Proof 必须校验签名、过期时间、solution、难度上限和一次性使用。
- replay key 使用 proof/challenge 派生摘要，不存储 payload 明文。
...
- challenge 生成缺配置、校验失败、replay 命中记录结构化 warn。
...
| 防重放      | Durable Object；KV fallback             | 现有 binding | 生产走 DO 原子消费，KV 仅用于本地/测试 fallback                    |
...
graph LR
  ...
  Verify --> Replay[(Durable Object replay key)]
```
...
sequenceDiagram
  ...
  Worker->>Worker: verify HMAC + solution + expiresAt
  Worker->>Replay: atomically consume replay key
...
├── altcha.ts      ← challenge 签发、payload 校验、replay 消费
...
| ALTCHA Widget v3 payload 与自实现校验不一致 | 低   | 高   | 测试使用 v3 payload 形态，按 `{ challenge, solution }` 校验 |
| replay 存储写入失败                         | 低   | 中   | fail closed，记录 warn                                      |
